...
int main() {
    ifstream fin("graphcoloring10000.in");
    ofstream fout("graphcoloring10000.out");
    ...
    // Color vertices using the approximation algorithm
    for (int i = 0; i < n; i++) {
        ...
        int c = 1;
        while (usedColors.find(c) != usedColors.end()) {
            c++;
        }
        color[vertex] = c;
    }
