...
if __name__ == '__main__':
    ...
    binCapacity = 10
    print("Bin Packing:")
    bins, bin_packing_time = binPackingParallel(items, binCapacity)
    for i, bin in enumerate(bins):
        print(f"Bin {i+1}: {bin['items']}")
    print("Bin Packing Time:", bin_packing_time)
    print()

    items = [(2, 3), (3, 4), (4, 5), (5, 6)]
    capacity = 8
    print("Knapsack:")
    max_value, selected_items, knapsack_time = knapsackParallel(items, capacity)
    print("Max Value:", max_value)
    print("Selected Items:", selected_items)
    print("Knapsack Time:", knapsack_time)
    print()

    graph = nx.Graph()
    graph.add_edges_from([(1, 2), (1, 3), (2, 3), (2, 4), (3, 4)])
    print("Graph Coloring:")
    coloring, coloring_time = graphColoringParallel(graph)
    print("Coloring:", coloring)
    print("Graph Coloring Time:", coloring_time)
    print()

    graph = nx.Graph()
    graph.add_edges_from([(1, 2), (2, 3), (3, 4), (4, 1)])
    print("Hamiltonian Cycle:")
    hamiltonian_cycle, hamiltonian_time = hamiltonianCycleParallel(graph)
    print("Hamiltonian Cycle:", hamiltonian_cycle)
    print("Hamiltonian Cycle Time:", hamiltonian_time)
    print()
