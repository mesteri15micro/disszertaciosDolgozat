...
int main() {
    int capacity;
    vector<Item> items = readItemsFromFile("knapsack10000_9.in", capacity);

    double elapsedTime;
    vector<Item> selectedItems = knapsackApproximation(items, capacity, elapsedTime);

    ofstream outputFile("knapsack10000_9.out");
    if (outputFile.is_open()) {
        ...
        outputFile.close();
        cout << "Output saved to 'knapsack10000_9.out'." << endl;
    }
    else {
